...
- New Prisma models: `EnvironmentRegistration` (ENV-001/002), `ReleaseRecord` (VER-003), `ReleaseReadiness` (QLT-004); `prisma db push` + generate applied. Zod schemas added to `validation.ts`.
- Release docs authored: `docs/RUN_GUIDE.md`, `docs/USER_GUIDE.md`, `docs/ARCHITECTURE_SUMMARY.md`. `scripts/package_release.py` builds the app and produces a reproducible `release/` bundle (185 files) with `CHECKSUMS.txt` (SHA-256) and `RELEASE_MANIFEST.json`.
- Added `app/src/__tests__/p9.test.ts` — 14 new cases; full suite now **172/172 PASS** (run from `app/`), including TEST-ENV-001/002, TEST-VER-003, TEST-QLT-004, TEST-NFR-007, and NFR-003 strict tsc (0 errors). `next build` compiles. preflight --completion READY: PHASE_09.
- Added `origin` remote (github.com/slikizik/Venture-Studio-OS) and pushed `develop`; `vso_git_sync.py resume` reports RESUME READY.
- Updated trackers: BUILD_MANIFEST (phase 9 COMPLETE), PROJECT_STATUS, REQUIREMENTS_TRACEABILITY (VER-003/ENV-001/ENV-002/NFR-007/QLT-004 PASS), EXECUTION_QUEUE (P09-001..007), EXECUTION_STATE, HERMES_ACTIVITY_LOG. All 9 build phases COMPLETE.

## 1.7.0 — Phase 08 testing, data portability, and hardening complete (2026-08-15)

- Phase 08 (Testing, Data Portability, and Hardening) implemented and verified: project export/import (DAT-001/002/003) in `app/src/lib/dataPort.ts` with a versioned JSON package, transactional import using generic deep id-rewriting (remaps `id`, every `*Id` FK, embedded id arrays, and unique business keys `decisionId`/`code`/`slug` deterministically per import); `validateProjectPackage` rejects unknown format, unsupported version (only `1`), non-JSON, and malformed packages WITHOUT writing anything. Added `restoreBackup` (DAT-005) to `app/src/lib/backup.ts` with a dependency-free integrity check (SQLite `SQLite format 3` magic bytes) and transactional replace, returning the restored byte count.
- Added `app/src/lib/isolation.ts` (ENV-003) with `assertNoCrossEnvLeak` / `isEnvIsolatedFrom` guarding shared db path, storage dir, and secret source between isolated environments. Added `app/src/lib/sanitize.ts` (NFR-006) — a dependency-free HTML sanitizer + safe Markdown renderer (escapes, strips `script`/`style`/`iframe`/`object`, neutralizes `on*` handlers and `javascript:` URLs). Added `app/src/lib/a11y.ts` (NFR-004: contrast ratio, keyboard-accessibility, control a11y checks) and `app/src/lib/perf.ts` (NFR-005: ≤2s measurement helper).
- Wired an accessible export affordance (DAT-001 UI): `app/src/app/api/projects/[id]/export/route.ts` returns the versioned package, and `app/src/app/projects/[id]/export-dialog.tsx` is a keyboard-reachable, labelled, aria-live export trigger (NFR-004).
- Added 3 Phase 08 test files (dat, env, nfr) — 30 new cases; full suite now **158/158 PASS**, including TEST-DAT-001/002/003/005, TEST-ENV-003, TEST-NFR-002/004/005/006/010, and NFR-003 strict tsc (no `any`). tsc --noEmit 0 errors; `next build` compiles. preflight --completion READY.
- No app code under the frozen architecture changed beyond these additions (no new infra, no new dependencies). No unregistered production stubs (PH-002/003/004 config placeholders remain owner-owned, non-blocking).
- Updated trackers: BUILD_MANIFEST (phase 8 COMPLETE, phase 9 READY), PROJECT_STATUS, REQUIREMENTS_TRACEABILITY (ENV-003, DAT-001/002/003/005, NFR-002/004/005/006/010 COMPLETE/PASS), EXECUTION_QUEUE (P08-001..007), EXECUTION_STATE, HERMES_ACTIVITY_LOG. Phase 09 (Packaging and Release) marked READY.
...
- Schema change: added `ProjectTemplate` + children (`TemplateStage`, `TemplateDeliverable`, `TemplateGate`, `TemplateBrainSection`, `QualityProfile`) and `Project.templates` back-relation; added `QualityProfile.version` / `isLatest`. Applied via `prisma db push --accept-data-loss --skip-generate` + regenerated client.
- Six built-in templates implemented and fail-fast validated at module load: blank, standard-venture, product-release, research, service, internal-tool.
- Added 5 API routes (templates, quality-profiles, quality-profiles/[id] DELETE, templates/custom, templates/custom/[id]) and 3 UI pages + clients (templates list, new custom template, quality profiles) wired into nav + project dashboard.
- Added 2 Phase 06 test files (tpl, qlt) — 17 new cases; full suite now **115/115 PASS**, including TPL-001/002/003, QLT-001, and NFR-003 strict tsc (no `any`). tsc --noEmit 0 errors; `next build` compiles. preflight --completion READY.
- Fixed real issues: SQLite single-connection deadlock (instantiateTemplate ran inside createProject's `$transaction` with the global client → moved instantiation/blank-stage/audit/seed-version outside the transaction); DeliverableDependency join-table-only storage (test asserted scalar column); QualityLevel string-enum array (test sent object array); createQualityProfile signature (single-arg vs projectId+input); NFR-003 `any` removal (typed to concrete interfaces). No scope change.
- Updated trackers: BUILD_MANIFEST (phase 6 COMPLETE, phase 7 READY), PROJECT_STATUS, REQUIREMENTS_TRACEABILITY (TPL-001/002/003, QLT-001 COMPLETE), EXECUTION_QUEUE (P06-001..008), EXECUTION_STATE, HERMES_ACTIVITY_LOG. Phase 07 (Dashboards and Environment Identity) marked READY.
