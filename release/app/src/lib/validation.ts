...
// ---- Phase 09: Environments (ENV-001/002) ----
export const EnvStatus = z.enum(["UP", "DOWN", "UNKNOWN", "REGISTERED"]);
export const environmentRegisterSchema = z.object({
  envId: z.string().min(1).max(64),
  name: z.string().min(1).max(150),
  branch: z.string().max(200).optional().nullable(),
  worktreePath: z.string().max(500).optional().nullable(),
  port: z.number().int().positive().max(65535).optional().nullable(),
  dbPath: z.string().max(500).optional().nullable(),
  storageDir: z.string().max(500).optional().nullable(),
  logPath: z.string().max(500).optional().nullable(),
  status: EnvStatus.default("REGISTERED"),
  notes: z.string().max(2000).optional().nullable(),
});
export type EnvironmentRegisterInput = z.infer<typeof environmentRegisterSchema>;
...
// ---- Phase 09: Release / Version (VER-003) ----
export const ApprovalStatus = z.enum(["PENDING", "APPROVED", "REJECTED"]);
export const MergeReadiness = z.enum(["READY", "BLOCKED", "UNKNOWN"]);
export const releaseRecordCreateSchema = z.object({
  projectId: z.string().min(1),
  versionLabel: z.string().min(1).max(100),
  baseVersionLabel: z.string().max(100).optional().nullable(),
  comparison: z.string().default("{}"),
  approvalStatus: ApprovalStatus.default("PENDING"),
  mergeReadiness: MergeReadiness.default("UNKNOWN"),
  mergeBlockers: z.array(z.string().max(500)).default([]),
  releaseStatus: z.string().max(40).default("NOT_READY"),
});
export type ReleaseRecordCreateInput = z.infer<typeof releaseRecordCreateSchema>;

export const releaseRecordApproveSchema = z.object({
  approvedBy: z.string().min(1),
  decision: ApprovalStatus,
});
export type ReleaseRecordApproveInput = z.infer<typeof releaseRecordApproveSchema>;

// ---- Phase 09: Release readiness state machine (QLT-004) ----
export const ReleaseReadinessState = z.enum([
  "NOT_READY",
  "TECHNICALLY_READY",
  "COMMERCIAL_REVIEW",
  "COMMERCIAL_READY",
...
export const releaseReadinessCreateSchema = z.object({
  projectId: z.string().min(1),
  readinessState: ReleaseReadinessState.default("NOT_READY"),
  technicalReady: z.boolean().default(false),
  commercialReady: z.boolean().default(false),
  technicalNotes: z.string().max(2000).default(""),
  commercialNotes: z.string().max(2000).default(""),
  blockingChecks: z.array(z.string().max(300)).default([]),
  releasedVersion: z.string().max(100).optional().nullable(),
});
export type ReleaseReadinessCreateInput = z.infer<typeof releaseReadinessCreateSchema>;

export const releaseReadinessEvaluateSchema = z.object({
...
export type ReleaseReadinessEvaluateInput = z.infer<typeof releaseReadinessEvaluateSchema>;
